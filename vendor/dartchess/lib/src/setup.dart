...
@immutable
class Pockets {
  const Pockets._(this._white, this._black);

  // Bitfield: 5 bits per role, one field per side. A single 60-bit field
  // would not survive on the web, where an int is a JavaScript number;
  // 6 roles x 5 bits = 30 bits per side fits everywhere.
  final int _white;
  final int _black;

  /// An empty pocket.
  static const empty = Pockets._(0, 0);

  static int _offset(Role role) => role.index * 5;

  int _sideValue(Side side) => side == Side.white ? _white : _black;

  /// Gets the number of pieces of that [Side] and [Role] in the pocket.
  int of(Side side, Role role) =>
      (_sideValue(side) >> _offset(role)) & 0x1F;
  ...
  Pockets increment(Side side, Role role) => side == Side.white
      ? Pockets._(_white + (1 << _offset(role)), _black)
      : Pockets._(_white, _black + (1 << _offset(role)));

  /// Decrements the number of pieces in the pocket of that [Side] and [Role].
  Pockets decrement(Side side, Role role) => side == Side.white
      ? Pockets._(_white - (1 << _offset(role)), _black)
      : Pockets._(_white, _black - (1 << _offset(role)));
  ...
  bool operator ==(Object other) {
    ...
    return other is Pockets &&
        other._white == _white &&
        other._black == _black;
  }

  @override
  int get hashCode => Object.hash(_white, _black);
}
