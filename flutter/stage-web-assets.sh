#!/usr/bin/env bash
# Stage the web assets that are copies rather than sources.
#
# vendor/{wasm,retro,garbo} are third-party engine builds we carry in the repo.
# They lived under static/ until the Svelte app was retired (2026-07-20), which
# is the only reason they were ever called "static".
#
#   web/brain.js   <- assets/brain.js   (built by `npm run build:brain`)
#   web/wasm/      <- ../vendor/wasm/   (the Stockfish WASM build we vendor)
#   web/retro/     <- ../vendor/retro/  (the historical engines, wasm + worker)
#   web/garbo/     <- ../vendor/garbo/  (Garbochess-JS 2011, worker + LICENSE)
...
# (vendor/retro/ is tracked), so this is only ever a copy, never a build.
rm -rf web/retro
cp -R ../vendor/retro web/retro
