...
describe('Dart family literals match the roster', () => {
	...
	it('every playable family reaches the roster as a real persona or a runtime one', () => {
		// The other direction: a family in the whitelist that no persona carries
		// and that is not user-added is a filter clause matching nothing.
		for (const f of dartStringsIn(families, MARKER)) {
			if (dynamicFamilies.has(f)) continue;
			expect(realFamilies, `whitelist lists '${f}', which no persona has`).toContain(f);
		}
	});
});
